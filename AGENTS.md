...
## 见闻 product decisions
- Selected visual truth: design-source.png, first displayed concept. Warm ivory, Song-style editorial headings, orange accents, fabric/steel hero.
- Personal iPhone news prototype: weekday 07:40 Rome time, 15 stories / 10 minutes. Saturday 15:00 weekly 3–5 opportunities. Europe, Southeast Asia, Japan, Korea, China.
- Tabs Today, Fable, Ideas. Saving an article from Today automatically adds it to Ideas; Ideas has no multi-stage status workflow.
- Prototype keeps a safe fallback feed, while `src/data/daily-feed.json` is now the build-time feed contract. GitHub Actions can collect RSS/Atom items, deduplicate them, preserve source links and dates, and optionally create Chinese summaries with `OPENAI_API_KEY`; it must not imply verified market evidence or investment advice.
- Updated interaction rule: reading an article only records reading progress; it never qualifies an item for saving. Every article has a manual save action, and saved articles are shown directly in Ideas for later reading.
- Updated navigation rule: Today, Fable, and Ideas are one tab shell with left/right transitions based on tab order. The tab bar is fixed to the iPhone safe-area bottom.
- Content freshness rule: fallback rows are labeled 示例内容. When the collector writes a live feed, every row must keep a source URL and published date; missing AI summarization must remain visibly raw rather than be presented as researched Chinese analysis. The workflow checks Europe/Rome time for weekday 07:40; iPhone push still requires the separate `push-server` HTTPS service.
- Visual freshness rule: Today uses the feed's `heroImage`, selected by Europe/Rome weekday from seven weekday banner backgrounds, with the feed's `weekdayLabel` rendered as visible overlay text. Article detail reuses the current daily image. Do not restore the fixed editorial hero. `public/images/app-icon.png` is the app identity asset used by favicon, Apple touch icon, manifest, and push notifications.
- Daily fable rule: the second bottom tab reads from the append-only `src/data/daily-fables.json` library and the `src/data/daily-fable-history.json` date-to-ID ledger. It assigns only an unused fable ID for each Europe/Rome calendar day; the daily workflow appends a generated or offline-library entry when the current library is exhausted, so a fable is never selected by modulo or repeated. Render the indirect story before the concept reveal, formal explanation, and practical observation. Keep the concept hidden until the reveal block.
- Today feed rule: render all 15 selected stories in order on the Today page; there is no start-reading CTA. Reddit Technology is an additional RSS reference source, and source diversity should keep at least one item per fetched source when enough items are available.
