...
- Daily fable rule: the second bottom tab reads from `src/data/daily-fables.json`, selects one entry by the Europe/Rome calendar day, and renders the indirect story before the concept reveal, formal explanation, and practical observation. Keep the concept hidden until the reveal block.
