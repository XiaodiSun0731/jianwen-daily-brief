...
- Tabs Today, Projects, Saved. Full project analysis and manual save, stage selection and notes.
...
- Updated interaction rule: reading an article only records reading progress; it never qualifies an item for saving. Every article has a manual save action. Saved items begin at 已收藏, and the weekly Projects tab only shows saved items manually moved to 准备优化. Saved is the user's collection; Projects is the weekly optimization shortlist.
- Updated navigation rule: Today, Projects, and Saved are one tab shell with left/right transitions based on tab order. The tab bar is fixed to the iPhone safe-area bottom, and the weekly project detail shows three suggested next steps.
...
- Daily fable rule: the fourth bottom tab reads from `src/data/daily-fables.json`, selects one entry by the Europe/Rome calendar day, and renders the indirect story before the concept reveal, formal explanation, and practical observation. Keep the concept hidden until the reveal block.
